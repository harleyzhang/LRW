...
/*
  Added by Honglei Zhang. This function save a sparse matrix in triplet format to a file
  */
int cs_save(FILE *f, const cs *A)
{
    ...
    if (nz < 0) {
        printf("The sparse matrix must be in Triplet format");
        return(-1);
    } else {
        for (p = 0 ; p < nz ; p++) {
            fprintf (f, "%d %d %g\n", Ai [p], Ap [p], Ax ? Ax [p] : 1) ;
...
/* save a compressed binary format, added by honglei */
int cs_save_compressed (FILE *f, const cs *A)
{
    if (!A) { printf ("Compressed sparse matrix is null. \n"); return(0); }
    if (A->nz!=-1) { printf ("To be saved sparse matrix is not in compressed format! \n"); return (0); }
...
/* load a compressed binary format, added by honglei */
cs *cs_load_compressed(FILE *f) {
    int m, n, nzmax, nz;
    //nz: should be -1
    fread(&nz, sizeof(int), 1, f);
    if(nz!=-1) {printf("The saved sparse matrix is not in compressed format!\n"); return(NULL); }
    fread(&m, sizeof(int), 1, f);
    fread(&n, sizeof(int), 1, f);
    fread(&nzmax, sizeof(int), 1, f);

    //alloc memory. in compressed format
    cs *A = cs_spalloc(m, n, nzmax, 1, 0);
    fread(A->p, sizeof(int), n+1, f);
    fread(A->i, sizeof(int), nzmax, f);
    fread(A->x, sizeof(double), nzmax, f);
...
{
    void *p2 ;
    //fix by honglei. realloc take size_t as input, should check max size_t, which is either long or unsigned long
    //*ok = !CS_OVERFLOW (n,size) ;	    /* guard against int overflow */
    *ok = !CS_OVERFLOW_LONG (n,size) ;	    /* guard against int overflow */
...
/* fnorm of a sparse matrix */
double cs_fnorm (const cs *A)
{
    int p, j, n, *Ap ;
    double *Ax,  norm = 0, s ;
    if (!A || !A->x) return (-1) ;		/* check inputs */
    ...
    for (j = 0 ; j < n ; j++)
    {
        for (s = 0, p = Ap [j] ; p < Ap [j+1] ; p++) s += Ax[p] * Ax[p] ;
        norm += s;
    }
    return (sqrt(norm)) ;
}

/* Inplace normalise columns of a sparse matrix */
int cs_normalise_c(cs *A, int pnorm)
{
    //first get L1 norm of each column
    int p, j, n, *Ap ;
    double *Ax,  s ;
    if (!A || !A->x) return (-1) ;		/* check inputs */
    n = A->n ; Ap = A->p ; Ax = A->x ;
    for (j = 0 ; j < n ; j++)
    {
        for (s = 0, p = Ap [j] ; p < Ap [j+1] ; p++) s += fabs (Ax [p]) ;
        if (s!=0)
            for (p = Ap [j] ; p < Ap [j+1] ; p++) Ax[p] = Ax[p]/s ;
    }
    return (1) ;
}

int cs_element_power(cs *A, double power)
{
    //first get 1 norm of each column
    int p, j, n, *Ap ;
    double *Ax;
    if (!A || !A->x) return (-1) ;		/* check inputs */
    n = A->n ; Ap = A->p ; Ax = A->x ;
    for (j = 0 ; j < n ; j++)
    {
        for (p = Ap [j] ; p < Ap [j+1] ; p++) Ax[p] = pow(Ax[p], power) ;
    }
    return (1) ;
}
