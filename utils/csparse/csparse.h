...
/* add two sparse matrix: alpha*A + beta*B */
cs *cs_add (const cs *A, const cs *B, double alpha, double beta) ;

int cs_cholsol (const cs *A, double *b, int order) ;
int cs_dupl (cs *A) ;

/* add a new entry to the triplet format matrix */
...
/* get 1-norm of matrix. max(sum(abs(A))) */
double cs_norm (const cs *A) ;

/* get f-norm of the matrix
  * A_F = sqrt(sum(A_ij^2))
  * return a nx1 double
  *   -1 if input error
  */
...
/* added by Honglei
 * normalise the matrix by column
 * only L1-norm normalisation at the moment
 * This is inplace operataiton. After the function call, A is changed;
 * return:
 *  -1 : invalid input
 *  1 : success
 */
...
/* added by Honglei for fast save and load. Note the file content is plain binary, which is not binary compatible*/
...
/* if values==0, data storate is not allocated, but index storage are allocated 
   When fully constructed, the memory usage are: int[n](column index),  
   int[nzmax)(data row pointer), data[nzmax] if values is not zero. 
*/
cs *cs_spalloc (int m, int n, int nzmax, int values, int triplet) ;

/* return NULL after done */
cs *cs_spfree (cs *A) ;
...
/* drop small values from the sparse matrix */
...
#define CS_OVERFLOW_LONG(n,size) (n > LONG_MAX / (int) size)
